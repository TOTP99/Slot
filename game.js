/**
 * 万锦老虎机 - 拆分版（index.html + style.css + game.js）
...
class SlotGame extends Phaser.Scene {
  ...
  createPanel(x, y, width, height, fill = UI.panel, alpha = 0.96, hideGroup = null, scaleGroup = null) {
  ...
  // 奖池条背景：随机撒点小星星，各自独立节奏地忽明忽暗闪烁（叠加发光混合模式）
  // 星星尺寸放大 150%
  createJackpotSparkle(cx, cy, w, h) {
    const count = 16;
    this.jackpotStars = [];
  ...
  // ========== 通用：圆角面板绘制（Graphics 复用，替代直角 Rectangle） ==========
  // 统一由此绘制所有需要圆角的按键底板，避免每处各自重复实现。
  drawRoundedPanel(gfx, w, h, radius, strokeColor, strokeWidth, fillColor, fillAlpha = 1) {
    gfx.clear();
  ...
  // ========== 拉杆上方：时间显示 + 简/繁切换 ==========
  // 上：时:分；下："简🕹️繁"，当前状态高亮。长按此处切换 简（默认放大）/ 繁（原始完整布局）。
  // 时间/切换控件本身固定不动、不参与缩放，也不随简/繁切换隐藏。
  // 拉杆上方空地：时间 + 简/繁切换按键（原长按 JACKPOT 的彩蛋功能移到这里，改为点击）
  // 圆角金色边框 + 暗色底，模拟实体按键质感；点按有轻微按下回弹反馈。
  createRightClockToggle(lx) {
    const btnSize = 70;
    const cy = 165;
    const clockY = cy - 16;
    const modeY = cy + 14;
  ...
  // 依据 focusMode 高亮当前状态：简（放大态，默认）/ 繁（完整态）
  refreshModeLabel() {
  ...
  // ========== 隐藏功能（原：长按 JACKPOT，已移至拉杆上方的时间/简繁按键，改为点击）==========
  // 触发后：左侧唱片面板隐藏；老虎机机身 + 下方四分播报屏作为一个整体一起放大 115%；
  // 时间/简繁按键与拉杆均保持原地不动、不参与缩放。再次点击恢复原状。
  ...
  // silent: 初始化时直接应用默认放大态，不出声、不播放过渡动画
  toggleFocusMode(silent) {
  ...
  // 赔率弹窗 + 速度 / 自动五次 / 音效 设置（点击"赔率*设置"键弹出）
  // 已去掉"控制中心/赔率/设置"文字标签与顶部分隔线；所有图标/字体/方框在原基础上放大 115%，
  // 面板本身也相应放大，尽量利用面板内的空间，避免大片空白。
  createSettingsModal() {
    const cx = GAME_WIDTH / 2;
    const cy = GAME_HEIGHT / 2;
    const panelW = 560;
  ...
  // 每个转轮停止时的最终结果：取用 startSpin() 时已按概率表决定好的这一把结果
  // （this.pendingSpinResult），而不是每个轮子各自独立随机
...
// ---------- 竖屏自动跳转：带丝滑过渡动画转去独立的留声机游戏 ----------
// 老虎机本身只做横屏，竖屏时不再自渲染任何 UI，而是过渡后整页跳转到 mp3 仓库。
(function setupPortraitRedirect() {
  var REDIRECT_URL = "https://totp99.github.io/mp3/";
  var overlay = document.getElementById("rotate-redirect");
  if (!overlay) return;
